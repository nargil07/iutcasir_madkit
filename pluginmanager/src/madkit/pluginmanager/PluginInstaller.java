...
"</html>");
				...
				choose.addActionListener(new ActionListener(){
					...
					public void actionPerformed(ActionEvent e) {
						...
						if(res==JFileChooser.APPROVE_OPTION){
							...
							if(dir!=null){
								if(!dir.exists()){
									int op=JOptionPane.showConfirmDialog(PluginInstaller.this,
											"The Selected Directory does not exists. Do you want to create it?",
								...
								if(!dir.canWrite()){
									JOptionPane.showMessageDialog(PluginInstaller.this,
											"Can't write in the selected directory",
											"Can not Write",
		...
		_index.addInstallStep(new InstallStep(4,"installconfig","Configure Installation"){
			...
			public boolean isStepFinished(){
				if(inet){
					...
					if(email.indexOf('@')<0){
						setWarningMessage("A valid email address is required");
						return false;
					}
		...
		_index.addInstallStep(new InstallStep(5,"done","Install"){
			...
			public void end(){
				...
				Object inetSource = _context.get("inetSource");
				String downServer= (inetSource == null ? null : inetSource.toString());
				...
				if(inet){					
					...
					try {
						...
						//System.out.println(">>>>> installation depuis un fichier (inet = "+inet+")");
				...
				if(full){
					//System.out.println(">>>>>> installation totale");
					agent.setFullInstallRequest();
				}
				
				if (!inet && full){
					_kernel.launchAgent(agent,"PluginManagerAgent",_kernel,false);
				}
				else 
				...
				if (!inet && full){
					AgentAddress manager = agent.getAddress();
					agent.sendMessage(manager, new SelectedPluginMessage("madkit-full"));
					agent.sendMessage(manager, new StringMessage("startInstall"));
				}
	...
	public void setupGUI(AbstractAgent arg0) {
		...
		if(agui!=null && agui instanceof Component){
			...
			java.net.URL u = this.getClass().getResource("/images/madkit_logo32.gif");
		     if (u!=null)	
		      	af.setIconImage(new ImageIcon(u).getImage());
			_guis.put(arg0,af);
			af.show();
		}
