...
/*
 import org.jext.event.*;  // a definir!!!
 import org.jext.gui.*;
 */

import gnu.gui.*;
import gnu.options.event.*;
...
public class JBrowse
  ...
  private
  /*Frame*/ AbstractEditorPanel frame; // passed by activator
...
//		}
    ...
    // Connect to the optionane's options
...
//     }
  ...
  public void actionPerformed(ActionEvent evt) {
    ...
    //$$ from Seb [[
    if ( evt.getActionCommand().equals("reparse") )
    {
    	parseNow();
		return;
	}
  ...
  public void setPreferredSize() {
    ...
    if (statusPanel.isVisible()) {
      ...
      newFrameWidth = Math.max(newFrameWidth,
                               statusPanel.getPreferredSize().width + 8);
    }

    // Adjust width for top panel
    dFrame.width = Math.max(newFrameWidth,
                            topPanel.getPreferredSize().width + 8);
...
//%					+ "\n	       memberName: " + memberName );
  ...
  class TreePane
    ...
    public TreePane(String title) {
      ...
      url = this.getClass().getResource("/images/Parse.gif");
      if (url == null) {
...
//			if (!fileName.toUpperCase().endsWith(".JAVA") ) {
  ...
  static class OptionDialog
    ...
    //---------------------------------------------------------------------
    OptionDialog( /*Frame*/AbstractEditorPanel jparent, JBrowse parent,
                           String title) {
