  ...
  public boolean usesJavaTokenMarker() {
    return (ls instanceof JEditLineSource) ?
        ( ( (JEditLineSource) ls).getTextArea().getTokenMarker()
         // $$$$ modif
         instanceof
         /*org.gjt.sp*/ jedit.lang.JavaTokenMarker)
        : false;
...
//		TokenSource ts = new TokenSource(ls, JBrowse.debugLevel );
    ...
    try {
      ...
      do {
        ...
        if (token == null) {
          break; // may be necessary if we end in a comment
        }
        ...
        if (parseState == ParseState.HEADER) {
          ...
          if (parseSubState == null) {
            if (token.equals("package")) {
              ...
              String lastMsgStr =
                  "Expecting package/import or package member definition.";
            ...
            if (token.equals("import")) {
              ...
              String lastMsgStr =
                  "Expecting import or package member definition.";
...
//								mod = RWModifier.setVolatile(mod);
            ...
            if (parentNode == root) {
              ...
              if (RWModifier.isPublic(mod)) {
                ...
                if (!className.equals(expectedTopLevelName)) {
                  ...
                  msgStr = "This top-level public " + eType +
                      " must be defined in file " + className + ".java";
          ...
          if (badFlag) {
            ...
            String lastMsgStr =
                "Expecting package member (class or interface).";
          ...
          if (token.equals("{")) {
            ...
            if (token.equals("extends")) {
              ...
              if (!extendsOK) {
                ...
                implementsOK = true;
                currentElement = new UML.Generalization(token,
                    UML.Type.IMPLEMENTS,
                    parentNode.getElement(), curTokenLine);
                insertAsNode(currentElement, curElementStartPos, parentNode);
          ...
          if (Character.isJavaIdentifierStart(tokenStartChar)) {
            ...
            if (memberType == null) {
              ...
              if (memberType.equals(parentNode.getElement().getName())) {
                ...
                if (parentNode.getElement().isInterface()) {
                  ...
                  badFlag = true;
                  msgStr =
                      "Constructor can't be native/abstract/static/synchronized/final";
                }
            ...
            if (!resetFlag) {
              ...
              if (parentNode.getElement().getParentElement().isInterface()
                ...
                String msgStr2 =
                    "Invalid modifiers for a method in an interface";
...
class ParseSubState {
  ...
  public ParseState getParentState() {
    return parentState;
  }

  public boolean equals(Object o) {
    if (! (o instanceof ParseSubState)) {
      return false;
    }
    return (this == o);
  }
